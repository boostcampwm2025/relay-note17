...
챌린지에서 배운 CS지식을 블로그에 정리하기 위해 ChatGPT를 활용해보았지만, 처음부터 만족스러운 결과물이 나오지는 않았다. 예를 들어 글의 말투가 내가 원하는 방향과 다르거나, 설명은 잘 되어 있었지만 예시 코드가 포함되지 않는 경우가 있었다.원하는 결과물을 얻기 위해 프롬프트를 2~3번 수정하고 반복 실행해야 했다.

이 과정에서 생성형 AI는 강력한 도구이지만, 그 효과는 사용자의 프롬프트 작성 능력에 따라 크게 달라진다는 점을 체감했다.
처음에는 막연하게 "블로그 글처럼 정리해줘" 정도로 요청했지만, 원하는 말투, 구성 방식, 포함되어야 할 내용 등을 구체적으로 명시했을 때 비로소 내가 원하는 결과를 얻을 수 있었다.

프롬프트를 작성하는 것도 하나의 역량이라는 점을 느꼈다.
처음에는 단순한 명령으로도 충분할 것이라 생각했지만, 점점 “말투는 자연스럽고 설명은 간결하게, 핵심 키워드는 강조하고, 코드 예시는 실제 동작 가능한 수준으로“ 등 내가 원하는 조건을 상세히 제시해야만 정확한 결과가 나오는 것을 경험했다.
